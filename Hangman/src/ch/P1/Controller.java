...
public class Controller implements ViewController{
	...
	@FXML protected Label solution;
	@FXML protected Label trys;
	...
	*/
	...
	public void pruefen(ActionEvent event){
		...
		for(int p = 0; p < word.length(); p++){
			if(button.charAt(0)+32 == word.charAt(p)){
				line = true;
				corect++;
				array[p] = button.charAt(0);
				String b = new String(array);
				solution.setText(b);
			}
		}
		...
		*/
		if(!line){
		    ...
		    */
	        wrongs--;
			if(wrongs==0){
				trys.setText("You are dead");
				solution.setText(word);
				Alert alert = new Alert(AlertType.CONFIRMATION);
				alert.setTitle("Restart");
